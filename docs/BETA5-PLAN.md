...
6. **Bio, added after review (2026-07-17).** ✅ **Done.** The same `app.bsky.actor.profile` record already read for the avatar carries a `description` field — read for free in the same request, no new call. `fetchAvatarURL` became `fetchBlueskyProfile`, returning both; `resolveIdentity` stays avatar/handle-only for call sites that don't need a bio (shelf rows, note bylines), while a new `resolveIdentityFull` exposes it for the profile page.
