...
**Beta 0 through Beta 6 — done ✅** (2026-07-15 through 2026-07-18). End to end, against the real network: OAuth login, authenticated write and delete, sync via Tap against the production relay, cross-account aggregation, a real TMDB-backed search, season/episode-level catalog browsing, notes anchored to a work or to a specific episode, a real interface (no more raw HTML strings) with resolved handles/avatars instead of raw DIDs, a persistent site shell (topbar, navigation), a real profile page for any account by handle, and — as of Beta 6 — a three-tab feed (Shelf, Following, Affinity-pending) with real reply threads and reposts on notes (attributed, never counted — no popularity metrics anywhere). See [`docs/BETA0-PLAN.md`](docs/BETA0-PLAN.md), [`docs/BETA1-PLAN.md`](docs/BETA1-PLAN.md), [`docs/BETA2-PLAN.md`](docs/BETA2-PLAN.md), [`docs/BETA3-PLAN.md`](docs/BETA3-PLAN.md), [`docs/BETA4-PLAN.md`](docs/BETA4-PLAN.md), [`docs/BETA5-PLAN.md`](docs/BETA5-PLAN.md), and [`docs/BETA6-PLAN.md`](docs/BETA6-PLAN.md), which document the whole process, not just the outcome.
...
- [`lexicons/social/orbita/note.json`](lexicons/social/orbita/note.json) — the second Lexicon, a note about a work as a whole or about one specific season/episode, with an optional `reply` field (root/parent, mirroring AT Protocol's own reply shape) for real threaded conversation
- [`lexicons/social/orbita/repost.json`](lexicons/social/orbita/repost.json) — the third Lexicon, a repost of a note; exists solely to carry it into a follower's feed, attributed — never counted, no popularity metric anywhere
- [`cmd/appview/`](cmd/appview/) — a Go server exposing a JSON API (`api.go`) behind real OAuth login: search-then-write (`/search`), shelf add/remove and notes writable straight from the work page, season/episode navigation with real TMDB synopses and each episode getting its own dedicated page, a profile for any account by handle (`/profile/{handle}`), a three-tab feed (`/feed` — Shelf, Following, Affinity) with reply threads and reposts, a Tap webhook indexing all three collections into a local SQLite database, and handle/avatar/bio resolution (`identity.go`) straight from each account's own DID document and PDS — never through Bluesky's API, so it works the same for any AT Protocol account
...
Next up: see [`docs/ROADMAP.md`](docs/ROADMAP.md) for the macro shape of what's left. The shelf as a creative space (Beta 7) is next, followed by constellation and archetype on the profile (Beta 8); forum, events, real fan-out, observability, affinity, direct messages, and closing known gaps all follow. All still a sketch, none started.
