...
**Beta 0 — quase lá.** 5 dos 7 itens do critério de conclusão prontos, incluindo os dois mais arriscados (login OAuth real, escrita autenticada confirmada na rede de produção) — ver [`docs/BETA0-PLAN.md`](docs/BETA0-PLAN.md), que continua vivo e é atualizado a cada passo real, não só na hora do planejamento.
...
O que falta pro Beta 0 ser considerado concluído (ver critério em `docs/BETA0-PLAN.md`): o Tap ainda aponta pro PDS local, não pro relay de produção — falta confirmar que ele pega o registro que já existe na conta real — e indexar o que o webhook recebe num banco local, com uma página simples listando o resultado.
